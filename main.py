...
class Hangman:
    ...
    def _gallow(self):
        stand = pygame.draw.rect(
            screen, self.gallow_color, pygame.Rect(75, 280, 120, 10)
        )
        body = pygame.draw.rect(
            screen, self.gallow_color, pygame.Rect(128, 40, 10, 240)
        )
        hanger = pygame.draw.rect(
            screen, self.gallow_color, pygame.Rect(128, 40, 80, 10)
        )
    ...
    def _man_pieces(self):
        if self.wrong_guess_count == 1:
            ...
            body = pygame.draw.rect(
                screen, self.body_color, pygame.Rect(206, 105, 8, 45)
            )
    ...
    def _right_guess(self, guess_letter):
        index_positions = [
            index for index, item in enumerate(self.secret_word) if item == guess_letter
        ]
        for i in index_positions:
            self.guessed_word = (
                self.guessed_word[0:i] + guess_letter + self.guessed_word[i + 1 :]
            )
    ...
    def _guess_taker(self, guess_letter):
        if guess_letter in ascii_letters:
            if (
                guess_letter in self.secret_word
                and guess_letter not in self.guessed_word
            ):
                self._right_guess(guess_letter)
            elif (
                guess_letter not in self.secret_word
                and guess_letter not in self.wrong_guess
            ):
    ...
    def _message(self):
        ...
        # lose situation
            ...
            word = self.font.render(
                f"secret word: {self.secret_word}", True, (255, 255, 255)
            )
    ...
    def main(self):
        ...
        instructions = self.font.render(
            "Press any key to take Guess", True, (9, 255, 78)
        )
        screen.blit(instructions, (35, 460))
        while self.running:
            # shows the guessed word in the game window
            guessed_word = self.font.render(
                f"guessed word: {self.guessed_word}", True, (0, 0, 138)
            )
            screen.blit(guessed_word, (10, 370))
            # shows the wrong guesses in the game window
            wrong_guesses = self.font.render(
                f"wrong guesses: {' '.join(map(str, self.wrong_guess))}",
                True,
                (125, 0, 0),
            )
